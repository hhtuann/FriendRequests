...
document.querySelectorAll(".decline-btn").forEach((button) => {
    button.addEventListener("click", function () {
        ...
        if (container && container.querySelectorAll(".friend").length === 0) {
            // Nếu không còn friend nào thì hiển thị thông báo
            let message = document.createElement("p");  // Tạo phần tử p mới
            message.classList.add("no-more-friends");  // Thêm lớp để style (nếu cần)
            message.textContent = "Bạn đã xem hết";  // Thêm nội dung
            container.appendChild(message);  // Thêm thông báo vào container
        }
    });
});
